...
%% Diese Werte Vorher festlegen
    ...
    max_eingriffzeit_dimple = 0.25; %in Sekunden, beschreibt die Dauer, die z.B. ein einzelner Dimple oder Verzahnung gefräst wird. (Der Abstand zwischen zwei Fräsungen soll fertigungsbedingt größer sein, als die Eingriffszeit)
...
path = uigetdir('D:\01_Versuchsdaten WP2.1\02_Kraftdaten\ISEO - Messing\PS4\',"Ordener der .tdms Dateien wählen");
path = append(path,'\');
datatype = 'tdms';
...
for i = 1:size(tdms_files)      %durchläuft die tdms Dateien im Ordner
    ...
    tp = find_peaks(temp_fx, trigger, est_total_cutting_time/dt, trigger_sensitivity);
