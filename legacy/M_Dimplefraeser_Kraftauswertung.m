...
%% Diese Werte Vorher festlegen
    ...
    n = 0.1;  % Zeit welche vor und nach Schnitt mit geplottet wird. Muss durch dt teilbar sein
    ...
    est_total_cutting_time = 3; %Geschätzte gesamteingriffszeit für alle Dimple in Sekunden
    max_eingriffzeit_dimple = 0.68; %in Sekunden, beschreibt die Dauer, die z.B. ein einzelner Dimple oder Verzahnung gefräst wird. (Der Abstand zwischen zwei Fräsungen soll fertigungsbedingt größer sein, als die Eingriffszeit)
    ...
    Fzer_mode = true;    %setze true, wenn die Verläufe von Fa geplottet werden sollen
...
for i = 1:size(tdms_files)      %durchläuft die tdms Dateien im Ordner
    ...
    if (not(isnan(int_start))||not(isnan(int_end)))
        temp_fx = temp_fx(int_start-n/dt:int_end+n/dt);
        temp_fy = temp_fy(int_start-n/dt:int_end+n/dt);
        temp_fz = temp_fz(int_start-n/dt:int_end+n/dt);
        est_total_cutting_time = int_end - int_start;
        tp = find_peaks(temp_fx, est_total_cutting_time, trigger_sensitivity);
    ...
    %         end
        ...
        % Zerspankraft Verlauf erstellen als Excel
        if Fzer_mode == true
            cur_excel_path = append(excel_path,'NEW_', tdms_name);
            if not(isfolder(cur_excel_path))
                mkdir(cur_excel_path);  %erstellen des Verzeichnis, falls noch nicht vorhanden
            end
            %Tabelle erstellen
            filepath_excel = append(cur_excel_path,'\Fzer_Verlauf_Excel.xlsx');
            Fzer = Fzer';
            Tabelle_Fzer = table(time,Fzer);
            writetable(Tabelle_Fzer,filepath_excel);
            pause(0.1);
    ...
    max_Fzer = [max_Fzer; dimple_mean];
    
end

%MW der Messungen bilden
mean_all = mean(max_Fzer(~isnan(max_Fzer)));
filenames = [filenames; NaN; "Mittelwert"; "Standartabweichung"];
max_Fzer = [max_Fzer; NaN; mean_all; NaN];
filepath_excel = append(excel_path,'\mean_Fzer_Excel.xlsx');
table_print = table(filenames, max_Fzer);
