...
% @author: dgl_bd
%%
function fn_NS_Dimplefraeser_KSS_Kraftauswertung(path)
close all
% clear 
clc;
...
for i = 1:size(tdms_files)      %durchläuft die tdms Dateien im Ordner
    ...
    [int_start, int_end, rought_left_boundry] = KSS_manual_find_interval(temp_filtered_fz, trigger);
    if (not(isnan(int_start))||not(isnan(int_end)))
        ...
        for j = 1:3     %durchläuft fx, fy und fz
            ...
            filtered_f = KSS_drift_correction(filtered_f,tp(1),tp(end));
...
if height(deep) <= maxDimensionsTable(1)-3
    tempNaNVec = nan(maxDimensionsTable(1)-2-height(deep),1);
end
filenames = [filenames; tempNaNVec; "Mittelwert"; "Standartabweichung"];
max_Fzer_deep = [deep; tempNaNVec; mean_all_deep; NaN];
max_Fzer_flat = [flat; tempNaNVec; mean_all_flat; NaN];
